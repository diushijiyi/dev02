...
public class UserController {
    ...
    public String findByPage(@ApiParam(defaultValue = "分页参数：当前页",example = "1") @RequestParam(defaultValue = "1") Integer pageNum,
                             @ApiParam(defaultValue = "分页参数：每页显示多少条数据",example = "5") @RequestParam(defaultValue = "5") Integer pageSize){
        return "findByPage:当前页："+pageNum+"，每页显示多少条数据："+pageSize;
    }
    @ApiOperation(value = "添加所有的用户",tags = "operator")
    @PostMapping ("add")
    public String add(@ApiParam(name = "username",value = "用户名，必选",required = true) String username,
                      @ApiParam("用户密码，必选") String password){
        return "user add:"+username+password;
    }
    @ApiOperation(value = "删除所有的用户",tags = "operator")
    @DeleteMapping("delete")
    public String delete(){
        return "user delete";
    }
    @ApiOperation(value = "修改所有的用户",tags = "operator")
    @PutMapping("update")
    ...
    public String getHead(HttpServletRequest request){
        ...
        return "user getHead"+authorization;
    }
